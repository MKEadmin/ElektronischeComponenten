################################################################################
# Migration from Zerynth Studio to Zerynth SDK.
#
# Created: 2022-05-19
#
################################################################################
...
*****************
    ...
    This module contains the driver for MAXIM MAX30102 pulse oximetry and heart-rate monitor module.
    The MAX30102 is capable of direct I2C communication and can be set on 3 different operating mode (`datasheet <https://datasheets.maximintegrated.com/en/ds/MAX30102.pdf>`_).

"""
import i2c

# MAX30102 default slave address
MAX30102_I2CADDR       = 0x57

# MAX30102 registry addresses
# Status
...
# Configuration
MAX30102_FIFO_CONF     = 0x08

MAX30102_MODE_CONF     = 0x09
MAX30102_SPO2_CONF     = 0x0A

MAX30102_LED_CONF_1    = 0x0C
MAX30102_LED_CONF_2    = 0x0D
MAX30102_LED_CONF_3    = 0x0E

MAX30102_PROX_MODE     = 0x10

MAX30102_MLM_CTRL_1    = 0x11
MAX30102_MLM_CTRL_2    = 0x12

# Temperature
MAX30102_TEMP_INT      = 0x1F
MAX30102_TEMP_FRA      = 0x20
MAX30102_TEMP_CONF     = 0x21

# Proximity function
MAX30102_PROX_THR      = 0x30
...
MAX30102_SR_400        = 0x3
MAX30102_SR_800        = 0x4
MAX30102_SR_1000       = 0x5
MAX30102_SR_1600       = 0x6
MAX30102_SR_3200       = 0x7
...
MAX30102_LED_NONE      = 0x0
MAX30102_LED_RED       = 0x1
MAX30102_LED_IR        = 0x2
MAX30102_LED_GREEN     = 0x3

MAX30102_LED_CURRENT = [
    MAX30102_LED_NONE,
    MAX30102_LED_RED,
    MAX30102_LED_IR,
    MAX30102_LED_GREEN
...
MAX30102_MODE_HR       = 0x2
MAX30102_MODE_SPO2     = 0x3
MAX30102_MODE_MULTILED = 0x7

MAX30102_MODE = [
    MAX30102_MODE_HR,
    MAX30102_MODE_SPO2,
    MAX30102_MODE_MULTILED
]

MAX30102_INT_FULL      = 0x80
MAX30102_INT_DATA      = 0x40
MAX30102_INT_ALC       = 0x20
MAX30102_INT_PROX      = 0x10
MAX30102_INT_TEMP      = 0x02
MAX30102_INT_PWR       = 0x01


MAX30102_INT = {
    "full": MAX30102_INT_FULL,
    "data": MAX30102_INT_DATA,
    "alc":  MAX30102_INT_ALC,
    "prox": MAX30102_INT_PROX,
    "temp": MAX30102_INT_TEMP,  # write in MAX30102_INT_ENABLE_2_
    "pwr":  MAX30102_INT_PWR
}
...
MAX30102_RANGE2048     = 0x0
MAX30102_RANGE4096     = 0x1
MAX30102_RANGE8192     = 0x2
MAX30102_RANGE16384    = 0x3
...
.. class:: MAX30102(i2cdrv, addr=0x57, clk=400000)

    Creates an intance of the MAX30102 class.
    ...
    Example: ::
        ...
        from components.max30102 import max30102

        ...

        m301 = max30102.MAX30102(I2C0)
        m301.init()
    ...
    def __init__(self, drvname, addr=MAX30102_I2CADDR, clk=400000):
    ...
    def init(self, mode = MAX30102_MODE_SPO2, adc_range = MAX30102_RANGE16384, sample_rate = MAX30102_SR_50, pulse_width = MAX3010_LED_PW_411, led_current = [0xFF,0xFF,0x00,0x00], proximity_thrs = 0, slot_multi = [0x00,0x00,0x00,0x00] ):
        """
...
.. method:: init(mode = 'spo2', adc_range = 3, sample_rate = 50, pulse_width = 411, led_current = [255,255,0,0], proximity_thrs = 0, slot_multi = [0,0,0,0] )
    ...
    Initialize the MAX30102. Default paramter values enable ``"spo2"`` - without proximity - mode with: maximum ADC range, sampling rate of 50 Hz, LED pulse width of 411us and maximum pulse amplitude for both red and IR LEDs.

    :param mode: set the operating state of the MAX30102, default mode is ``spo2``.
...
--------------------
    ...
    The MAX30102 stores the digital output data in a 32-deep circular FIFO within the IC. The sample size depends on the number of LED configured as active. As each led signal is stored as a 3-byte data, the FIFO width can be 3, 6, 9 or 12 bytes in size.
...
.. method:: set_sample_averaging(n)
    ...
    Accepted values for ``n`` are: 1,2,4,8,16 and 32.
        ...
        reg = self.write_read(bytearray([MAX30102_FIFO_CONF,]),1)
        reg = reg[0] & 0x1F
        if n in MAX30102_AVG:
            val = n
        else:
            val = 0
        reg = reg | val<<5
        self.write(bytearray([MAX30102_FIFO_CONF,reg]))
...
.. method:: set_fifo_rollover(ro = True)
    ...
    This method controls the behavior of the FIFO when the FIFO becomes completely filled with data. If ``ro`` is ``True``, the FIFO Address rolls over to zero and the FIFO continues to fill with new data. If ``ro`` is ``False``, then the FIFO is not updated until FIFO is read or the FIFO WRITE/READ pointer positions are changed.
        ...
        if ro:
            reg = reg | 0x10
        self.write(bytearray([MAX30102_FIFO_CONF,]))
...
.. method:: read_raw_samples(nbyte)
    ...
    Return a nbyte-long bytearray containing raw data read from the FIFO.
        ...
        """
        v = self.write_read(bytearray([MAX30102_FIFO_DATA,nbyte]))
        return v
...
.. method:: clear_fifo()
    ...
    This method set to zero the FIFO read and write pointers and the overflow counter.
        ...
        """
        self.write(bytearray([MAX30102_FIFO_RD_PTR,0x00]))
        self.write(bytearray([MAX30102_OVF_COUNTER,0x00]))
        self.write(bytearray([MAX30102_FIFO_WR_PTR,0x00]))
...
* ``"temp"`` : when an internal die temperature conversion is finished, this interrupt is triggered so the processor can read the temperature data registers.
        ...
        try:
            for ss in sources:
                if ss in MAX30102_INT:
                    if MAX30102_INT[ss] == MAX30102_INT_PWR:
                        raise ValueError
                    if MAX30102_INT[ss] == MAX30102_INT_TEMP:
                        self.write(bytearray([MAX30102_INT_ENABLE_2,0x02]))
                    else:
                        reg = self.write_read(bytearray([MAX30102_INT_ENABLE_1,]),1)
                        val = MAX30102_INT[ss]
                        reg = reg[0] | val
                        self.write(bytearray([MAX30102_INT_ENABLE_1,reg]))
...
.. method:: read_triggered_interrupt()
    ...
    .. note:: interrupt defines are shown in method above; this function can return "pwr" interrupt (triggered on every power-up) that is enabled by default and cannot be disabled.
        ...
        """
        interrupts = []
        data = self.write_read(MAX30102_INT_STATUS_1,2)
        for k,v in MAX30102_INT.items():
            if v == MAX30102_INT_TEMP:
                res = data[1] & v
            else:
                res = data[0] & v
            if res:
...
.. method:: disable_interrupts()
    ...
    Disable all interrupts.
        """
        self.write(bytearray([MAX30102_INT_ENABLE_1,0x00]))
        self.write(bytearray([MAX30102_INT_ENABLE_2,0x00]))
...
.. method:: shutdown()
    ...
    Put the part into a power-save mode. While in power-save mode, all registers retain their values, and write/read operations function as normal. All interrupts are cleared to zero in this mode.
        ...
        """
        reg = self.write_read(bytearray([MAX30102_MODE_CONF,]),1)
        reg = reg[0] & 0x7F
        reg = reg | MODE_SHDN
        self.write(bytearray([MAX30102_MODE_CONF, reg]))
...
.. method:: wake_up()
    ...
    Wake up the MAX30102 component.
...
.. method:: reset()
    ...
    All configuration, threshold, and data registers are reset to their power-on-state through a power-on reset.
        ...
        """
        self.write(bytearray([MAX30102_MODE_CONF, MODE_RESET]))
        cnt = 0
        while cnt < 5:
            res = self.write_read(bytearray([MAX30102_MODE_CONF,]),1)[0]
            if (res & MODE_RESET) == 0:
                return True
            cnt += 1
            sleep(10)
...
.. method:: set_mode(mode, adc_range, sample_rate, pulse_width, led_current, proximity_thrs, slot_multi)
    ...
    Set the operating mode of the MAX30102. Default paramter values are the same of :func:`init()`.

**Parameters:**

* *mode* : set the operating state of the MAX30102. Available values are:
...
# SPO2 configuration
    def _set_adc_rge(self,n):
        reg = self.write_read(bytearray([MAX30102_SPO2_CONF,]),1)
        reg = reg[0] & 0x9F
        if n >= 0 and n <= 3:
            reg = reg | n<<5
        self.write(bytearray([MAX30102_SPO2_CONF,reg]))

    def _set_sample_rate(self,v):
        reg = self.write_read(bytearray([MAX30102_SPO2_CONF,]),1)
        reg = reg[0] & 0xE3
        if v in MAX30102_SAMPLE_RATE:
            reg = reg | v<<2
        self.write(bytearray([MAX30102_SPO2_CONF,reg]))
...
# LED Pulse amplitude configuration
    def _set_led(self,led):
        self.write(bytearray([MAX30102_LED_CONF_1,led[0]]))  # red
        self.write(bytearray([MAX30102_LED_CONF_2,led[1]]))  # ir
        self.write(bytearray([MAX30102_LED_CONF_3,led[2]]))  # green
        self.write(bytearray([MAX30102_PROX_MODE,led[3]]))   # pilot

# Proximity mode threshold configuration
    def _set_prox_thr(self,v):
        self.write(bytearray([MAX30102_PROX_THR,v]))

# Multi led mode configuration
    def _set_multi_slots(self,slot):
        reg1 = slot[1]<<4 | slot[0]
        self.write(bytearray([MAX30102_MLM_CTRL_1,reg1]))
        reg2 = slot[3]<<4 | slot[2]
        self.write(bytearray([MAX30102_MLM_CTRL_2,reg2]))
...
.. method:: enable_temperature()
    ...
    Initiates a single temperature reading from the temperature sensor.
        ...
        """
        self.write(bytearray([MAX30102_TEMP_CONF,0x01]))
...
.. method:: get_temperature()
    ...
    Returns a float representing the last temperature reading in Celsius.
        ...
        """
        t_reg_val = self.write_read(bytearray([MAX30102_TEMP_INT,]), 2)
...
.. method:: get_part_id()
    ...
    Returns the Part ID of the component.
        ...
        """
        data = self.write_read(bytearray([MAX30102_PART_ID,]),1)
        return data[0]
...
.. method:: get_revision_id()
    ...
    Returns the Revision ID of the component.
        ...
        """
        data = self.write_read(bytearray([MAX30102_REV_ID,]),1)
        return data[0]
